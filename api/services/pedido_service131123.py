...
def cadastrar_pedido(pedido):
    try:
        ...
        # Associe os produtos, cliente e fornecedor ao pedido
        pedido_bd.produtos = pedido.produtos
        pedido_bd.clientes = pedido.clientes
        pedido_bd.fornecedores = pedido.fornecedores
        ...
        print(e)
        raise ValueError(f"Erro ao cadastrar pedido: {str(e)}")


def cadastrar_pedido999(pedido):
    ...
    try:         # Cria uma instância do modelo Pedido com os valores fornecidos no objeto 'pedido'
        pedido_bd = pedido_model.Pedido(qtde_pedido=pedido.qtde_pedido, data_pedido=func.now(), data_entrega=pedido.data_entrega,
                                        status=pedido.status, obs=pedido.obs, cadastrado_em=func.now(), atualizado_em=pedido.atualizado_em, produtos=pedido.produtos,
                                        fornecedor_id=pedido.fornecedores, cliente_id=pedido.clientes, produto_id=pedido.produtos)
        ...
        print(e)
        raise ValueError("Estoque insuficiente para {produto.nome}" + str(e))

...
from ..schemas import pedido_schemas, receita_schema
...
class PedidoCForm(FlaskForm):
    ...
    filial_pdv = SelectField('Receita', validators=[DataRequired()])
    fornecedor_id = SelectField('Fornecedor', validators=[DataRequired()])
    # tipo = SelectField('Pedido p/', choices=[('compra', 'Compra'), ('producao', 'Produção')], validators=[DataRequired()])
    ...
    # metodo personalizado no seu formulário para extrair os dados do formulário em um formato serializável, como um dicionário.
    def to_dict(self):
        return {
            ...
            # 'tipo': self.tipo.data,# Se necessário, adicione novamente o campo 'tipo'
        }
...
@app.route('/pedidos/<int:id>', methods=['GET', 'PUT'])
...
@app.route('/pedidos/formulario', methods=['GET', 'POST', 'PUT'])
def fazer_pedido_compra():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            return redirect(url_for("listar_pedidos", form=form, form_data=form_data, pedido=pedido_bd))
        except ValidationError as error:
            return render_template('pedidos/formpedido.html', form=form, error_message=error.messages)
    else:
        return render_template('pedidos/formpedido.html', form=form, error_message=form.errors)
...
# TODO ** Classe PedidoProdução com os metodos e funcoes de views.
#     @author Fabiano Faria
